...
//  Su base de datos de passwords parece estar un poco dañada: algunas de las passwords no habrían sido permitidas por la Política Corporativa Oficial de Tobogán que estaba en vigor cuando fueron elegidas.
// Para intentar depurar el problema, han creado una lista (la entrada de su rompecabezas) de passwords (de acuerdo con la base de datos dañada) y la política corporativa cuando se estableció esa contraseña.
// Por ejemplo, suponga que tiene la siguiente lista:
// 1-3 a: abcde
//  1-3 b: cdefg
// 2-9 c: ccccccccc
// Cada línea proporciona la política de passwords y luego la contraseña.
//  La política de passwords indica el número más bajo y más alto de veces que debe aparecer una letra determinada para que la contraseña sea válida. Por ejemplo, 1-3 a significa que la contraseña debe contener al menos 1 vez y como máximo 3 veces.
//  En el ejemplo anterior, 2 passwords son válidas.
// La contraseña del medio, cdefg, no lo es; no contiene instancias de b, pero necesita al menos 1.
// La primera y tercera passwords son válidas: contienen una a o nueve c, ambas dentro de los límites de sus respectivas políticas. ¿Cuántas passwords son válidas según sus políticas?

const passwords = `3-11 z: zzzzzdzzzzlzz
3-7 x: xjxbgpxxgtx
3-4 v: vvmv
3-5 t: tgkfq
...
const arrPasswords = passwords.split('\n');
let correctPassword = 0;

const checkPassword = (txt) => {
  txt = txt.split(" ");
  const num = txt[0].split("-");
  const lessNum = num[0];
  const greaterNum = num[1];
  const letter = txt[1][0];
  const password = txt[2];
  let count = 0;
  ...
  if(count >= lessNum && count <= greaterNum) {
      correctPassword++;
  }
};

const numCorrect = (arrPasswords) => {
    for (password of arrPasswords) {
...
console.log(numCorrect(arrPasswords));
